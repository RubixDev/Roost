...
fn fetch() -> Result<(), Box<dyn std::error::Error>> {
    ...
    json_theme.insert(
        String::from("name"),
        theme_body.get("name").unwrap().clone(),
    );
            ...
            map.insert(
                ...
                JsonValue::Object({
                    ...
                    map.insert(
                        ...
                        colors.get("editor.background").unwrap().clone(),
                    );
                    map.insert(
                        String::from("foreground"),
                        colors.get("editor.foreground").unwrap().clone(),
                    );
                    map
                }),
            );
...
fn as_plist_val(value: &JsonValue) -> PlistValue {
    match value {
        ...
        JsonValue::Object(val) => {
            PlistValue::Dictionary(val.iter().map(|(k, v)| (k, as_plist_val(v))).collect())
        }
    }
}
